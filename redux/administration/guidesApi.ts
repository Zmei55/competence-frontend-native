...
const guidesApi = api.injectEndpoints({
	endpoints: (builder) => ({
		...
		updateIndustryById: builder.mutation<TIndustry, { id: number | string; name: string }>({
			query: ({ id, name }) => ({
				url: `/api/industry/${id}`,
				method: 'PUT',
				body: {
		...
		updateJobTitleById: builder.mutation<TJobTitle, { id: number | string; name: string }>({
			query: ({ id, name }) => ({
		...
		updateProfessionById: builder.mutation<TProfession, { id: number | string; name: string }>({
			query: ({ id, name }) => ({
