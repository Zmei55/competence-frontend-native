...
import { TCompetaConfirmationRegistered, TConfirmationFilter, THandleFilter } from '..';
import { CONFIRMED, DEFAULT_STRING, NOT_CONFIRMED, REJECTED } from 'shared';

export const useConfirmationFilter = (initialList: TCompetaConfirmationRegistered[] | null) => {
	const [confirmationFilterState, setConfirmationFilterState] = useState<TConfirmationFilter>({
		statusConfirmation: DEFAULT_STRING,
		sortByDate: DEFAULT_STRING,
	});
	const handleConfirmationFilter = ({ target: { name, value } }: THandleFilter) => {
	...
	let filteredConfirmations: TCompetaConfirmationRegistered[] | null = initialList;
	...
	if (filteredConfirmations !== null && confirmationFilterState.statusConfirmation === CONFIRMED) {
	...
	if (filteredConfirmations !== null && confirmationFilterState.statusConfirmation === REJECTED) {
		filteredConfirmations = filteredConfirmations.filter(
			(c) => c.statusCompetaConfirmation === REJECTED
		);
	}

	if (filteredConfirmations !== null && confirmationFilterState.sortByDate === DEFAULT_STRING) {
		if (confirmationFilterState.statusConfirmation === DEFAULT_STRING) {
			filteredConfirmations = initialList;
		} else {
			filteredConfirmations = filteredConfirmations.filter(
				(c) => c.statusCompetaConfirmation === confirmationFilterState.statusConfirmation
			);
		}
	}
