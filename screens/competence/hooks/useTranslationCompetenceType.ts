...
export const useTranslationCompetenceType = () => {
	...
	const translationCompetenceType = (type: string | null) => {
		...
		if (type === HARDSKILL_CUSTOM) return t('competenceType.hardSkill', { context: 'CUSTOM' });
		if (type === HARDSKILL_LANGUAGE) return t('competenceType.hardSkill', { context: 'LANGUAGE' });
